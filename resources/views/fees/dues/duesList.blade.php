                 ...
                 <tbody>
                     ...
                     @foreach($data as $key=> $stu)
                      ...
                      <tr>
                          ...
                          <input type='checkbox' class='checkbox_id pointer' name='checkbox[]' value="{{ $stu['id'] ?? ''}} " />
                         @php
                                     ...
                                     if(($search['class_type_id'] ?? '') != '')                                    
                                        {
                                            $feesGroups = DB::table('fees_master')
                                                ...
                                                ->whereNull('fees_group.deleted_at')
                                                ->whereNull('fees_master.deleted_at')
                                                ->groupBy('fees_master.fees_group_id')
                                                ->select('fees_master.*', 'fees_group.name as group_name','fees_master.id as fees_master_id','fees_group.id as fees_group_ids') 
                                                ->where('fees_master.class_type_id', $search['class_type_id'] ?? '')
                                                ->get();
                                            }
                                          ...
                                          where('fees_group_id', $item1->fees_group_ids)->where('admission_id',$stu['admission_id'])->sum('fees_group_amount');
                                         
                                          $head = DB::table('fees_detail')
                                          ->where('session_id',Session::get('session_id'))
                                          ->where('admission_id',$stu['admission_id'])
                                          ->where('fees_group_id',$item1->fees_group_id)
                                          ->whereNull('deleted_at')->whereIn('status', [0,1])->sum('total_amount');
