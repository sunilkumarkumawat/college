...
<style>
 ...
 <div class="col-md-12" >
 <div class="row">
         <div class="col-8 col-md-6 p-0 pr-1">
        <div class="card">
            ...
            <div class="card-body"> 
                ...
                <div class=" text-right d-flex" style='justify-content: end; align-items: end;'>
                 ...
                 <div class="col-md-4"> {{ __('Select Head') }}*</div>      
                 <div class="col-md-3"> {{ __('common.Amount') }}* </div>      
                 <div class="col-md-3"> {{ __('Discount') }} </div>      
                 <div class="col-md-2"> {{ __('Fine') }} </div>      
                 @if(!empty($fees_assign_details))
                        ...
                        @php
                                ...
                                $result = DB::table('fees_detail')
                                        ...
                                        ->where('fees_group_id', $fees->fees_group_id)
                                        ->whereIn('status',[0,1])
                                        ->first();
                      ...
                      <div class="col-md-4">
                            <input type="checkbox" class="selected_head pointer" id="checkbox_{{$key}}" name="selected_head[]" data-fees_assign_detail_id='{{$fees->id}}' value="{{$fees->fees_group_id}}">
                        ...
                        <div class="col-md-3">
                            ...
                            <input type="tel" class="form-control  amount_get aggregate_{{$fees->fees_group_id}}"  placeholder="{{ __('common.Amount') }}" id="amount_{{$fees->id}}" name="amount[]" onkeypress="javascript:return isNumber(event)"  required>
                    ...
                    <table id='fee_structure'class="table table-bordered padding_table" style="white-space:nowrap;">
                        ...
                        <tbody>
                            ...
                            @php
                                ...
                                $feesGroup = DB::table('fees_group')->whereNull('deleted_at')->where('id',$fees->fees_group_id)->first();
                                    $discount = DB::table('fees_detail')->whereNull('deleted_at')->where('admission_id',$fees->admission_id)->where('fees_group_id',$fees->fees_group_id)->whereIn('status',[0,1])->sum('discount');
                                  
                            @endphp
                            <div class="col-md-12">
                                <tr id="group_{{$fees->fees_group_id ?? ''}}" class='group_group' >
                                    ...
                                    <td>
                                        @if(!empty($feesGroup))
                                            @php
                                                $paid = DB::table('fees_detail')->whereNull('deleted_at')->where('admission_id',$fees->admission_id)->where('fees_group_id',$fees->fees_group_id)->whereIn('status',[0,1])->sum('total_amount');
                                                $paid_fine = DB::table('fees_detail')->whereNull('deleted_at')->where('admission_id',$fees->admission_id)->where('fees_group_id',$fees->fees_group_id)->whereIn('status',[0,1])->sum('installment_fine');
                                               @endphp                              
                                        @endif
                                    ...
                                    @php
                                    $headArray[] = ['pending_by_group_id'=>$fees->fees_group_id ?? '','pending'=>($pending_amount ?? 0)]
                                    @endphp
        ...
        @endphp
            <tr>
                ...
                <td>
                  ...
                  @if($val->status != 2)             
                    <!--<a href="{{url('student_assign_fees_edit',$val->admission_id)}}" target="blank" class="btn btn-primary  btn-xs" title="Edit Fees"><i class="fa fa-edit"></i></a>-->
...
<script>
    ...
    function updateTotals() {
        ...
        $("#aggregate").text(totalAmount.toFixed(2));
        $("#d_given").text(totalDiscount.toFixed(2));
        $("#g_total").text((totalAmount - totalDiscount).toFixed(2)); // Grand Total Calculation
    }
</script>
