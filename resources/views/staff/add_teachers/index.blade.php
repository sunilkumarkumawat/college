            ...
            <div class="card card-outline card-orange">
				<div class="card-header bg-primary flex_items_toggel">
					...
					<div class="card-tools">
			            ...
			            @endif
			            @if(Session::get('role_id') == 3)
			            <a href="{{url('dashboard')}}" class="btn btn-primary  btn-sm" title="Back"><i class="fa fa-arrow-left"></i> <span>{{ __('common.Back') }} </span></a> 
			            @endif
			        </div>
				</div>   
                            ...
                            <td>
                                ...
                                @if(isset($item->class_type_id) && !empty($item->class_type_id))
                                    @php
                                        ...
                                        $assignedClasses = App\Models\ClassType::with('course')
                                                    ->whereIn('id', $classIds)
                                        ...
                                        $groupedAssigned = $assignedClasses->groupBy(function($c) {
                                            return $c->course->name ?? 'Other / General';
                                        });
                                    @endphp
                                    
                                    @foreach($groupedAssigned as $courseName => $classes)
                                        <div class="mb-2 p-1" style="background: #f4f6f9; border-left: 3px solid #007bff; border-radius: 4px;">
                                            <strong class="text-dark" style="font-size: 11px;"><i class="fa fa-graduation-cap text-primary"></i> {{ $courseName }}</strong>
                                            <div class="mt-1">
                                                @foreach($classes as $c)
                                                    <span class="badge badge-light border text-dark" style="font-size: 10px; margin: 1px;">{{ $c->name }}</span>
                                                @endforeach
                                            </div>
                                        </div>
                                    @endforeach
                                @else
                                    <span class="badge badge-secondary font-italic" style="font-size: 11px;">Not Assigned</span>
                                @endif
