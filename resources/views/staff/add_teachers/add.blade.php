            ...
            <form id="quickForm" action="{{ url('teachers/add') }}" method="post" enctype="multipart/form-data">
               ...
               <div class="row m-2">
                  ...
                  <div class="col-md-4 col-12">
                     <div class="form-group">
                        <label>{{ __('Class / Semester') }} (Assigned)</label>
                        <select class="form-control select2" id="class_type_id" name="class_type_id[]" multiple data-placeholder="Select Semesters / Classes">
                           @if(!empty($getclassType)) 
                           @php
                               $groupedClasses = $getclassType->groupBy(function($item) {
                                   return $item->course->name ?? 'Other / General';
                               });
                               $oldClassIds = old('class_type_id', []);
                               if(!is_array($oldClassIds)) { $oldClassIds = [$oldClassIds]; }
                           @endphp
                           @foreach($groupedClasses as $courseName => $classes)
                           <optgroup label="{{ $courseName }}">
                               @foreach($classes as $value)
                               <option value="{{ $value->id }}" {{ in_array($value->id, $oldClassIds) ? 'selected' : '' }}>{{ $value->name ?? '' }}</option>
                               @endforeach
                           </optgroup>
                           @endforeach
                           @endif
               ...
               <div class="col-md-12 text-center">
                  <button type="submit" id="submitButton" class="btn btn-primary ">{{ __('common.submit') }}</button><br><br>
               </div>
            </form>
         </div>
      </div>
