                  ...
                  <tbody id="trColor">
                            @if(!empty($data))
                                ...
                                @foreach ($data  as $item)
                                  @php
                                  //dd($item);
                                $character = DB::table('evente_certificates')->where('admission_id',$item->id)->whereNull('deleted_at')->count();
                              //dd($character);
                              @endphp
                                <tr style="cursor:pointer; " class="fillData" data-id="{{ $item['id'] ?? '' }}" data-admission_type_id="{{$item['admission_type_id'] ?? ''}}" data-old="{{$character ?? '0'}}" data-school_name="{{ $getSetting['name'] ?? '' }}" data-class_type_id="{{ $item['class_type_id'] ?? '' }}" data-registration_no="{{ $item['admissionNo'] ?? '' }}" 
