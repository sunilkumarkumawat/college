                ...
                <div class="col-12 col-md-12">
                    <div class="card card-outline card-orange">
                        <div class="card-header bg-primary">
                            <h3 class="card-title"><i class="fa fa-image"></i> &nbsp;{{ __('master.View Gallery') }} </h3>
                            <div class="card-tools">
                                @if(Session::get('role_id') == 1)
                                <a href="{{url('gallery_add')}}" class="btn btn-primary  btn-sm {{ ($getPermission->add == 1) ? '' : 'd-none'}}" title="Add Gallary"><i class="fa fa-plus"></i>{{ __('Add Gallary') }}</a>
                                @endif
                                 @if(Session::get('role_id') == 1)
                                  <a href="{{url('master_dashboard')}}" class="btn btn-primary  btn-sm " title="Add Gallary"><i class="fa fa-arrow-left"></i>{{ __('common.Back') }} </a>
                                @endif
        ...
        <div class="row mt-3">
            @foreach($gallery_details as $data)
                 <div class="col-md-2">
                                    ...
                                    class="img-fluid gallery_img" onerror="this.src='{{ env('IMAGE_SHOW_PATH').'/default/user_image.jpg' }}'">
                                    @if(Session::get('role_id') == 1)
                                    <div class="edit_gallery_overley_div {{ ($getPermission->add == 1) ? '' : 'd-none'}}">
                                       <a href="{{url('gallery_edit')}}/{{$data->id}}"><button class="btn btn-primary Edit_Text">{{__('common.Edit') }}</button></a>
                                       <a><button class="delete_btn btn btn-danger ml-2 Edit_Text" data-id="{{$data->id}}" data-toggle="modal" data-target="#deleteModal">{{__('common.Delete') }}</button></a>
                                    </div>
                                    @endif
                                    </div>
                                </div>
